...
import { voteProposal, executeProposal } from "../../api/api";

const ProposalList = ({ proposals, userAddress, onUpdated }) => {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "28px" }}>
      ...
      {proposals.map((p) => (
        ...
        >
          ...
          >
            <span role="img" aria-label="proposal" style={{ marginRight: "8px" }}>
              📜
            </span>
            {p.description}
          </div>
          ...
          <div
            style={{
              color: p.executed ? "#4caf50" : "#e11d48",
              fontWeight: 700,
              marginBottom: "8px",
            }}
          >
          ...
          >
            {/* Only allow voting if proposal is active */}
            {Date.now() / 1000 >= p.startTime &&
            Date.now() / 1000 <= p.endTime &&
            p.proposer !== userAddress ? (
              <>
                <button
            ...
            {/* Only allow executing if voting ended and not yet executed */}
